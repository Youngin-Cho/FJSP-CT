...
from Agent.Common.ppo import Agent
from MARL.CTDE.validate import evaluate
...
def get_config():
    ...
    parser.add_argument("--critic_model_path", type=str, default=None, help="critic model file path")
...
def train(config):
    ...
    agent = Agent(fjsp_meta_data=env.fjsp_meta_data,
                  ...
                  ct_meta_data=env.ct_meta_data,
                  ct_state_size=env.ct_state_size,
                  ct_num_nodes=env.ct_num_nodes,
                  global_meta_data=env.global_meta_data,
    ...
    if use_saved_model:
        ...
        agent.fjsp_network.load_state_dict(fjsp_checkpoint['model_state_dict'])
        agent.fjsp_optimizer.load_state_dict(fjsp_checkpoint['optimizer_state_dict'])

        ct_checkpoint = torch.load(ct_model_path)
        agent.ct_network.load_state_dict(ct_checkpoint['model_state_dict'])
        agent.ct_optimizer.load_state_dict(ct_checkpoint['optimizer_state_dict'])

        critic_checkpoint = torch.load(critic_model_path)
        agent.global_critic.load_state_dict(critic_checkpoint['model_state_dict'])
    ...
    with open(log_dir + "critic_train_log.csv", 'w') as f:
        f.write('episode, reward, loss, lr\n')
    ...
    for e in range(1, num_episodes + 1):
        if use_vessl:
            vessl.log(payload={"FJSP_Train/LearnigRate": agent.fjsp_scheduler.get_last_lr()[0]}, step=e)
            vessl.log(payload={"CT_Train/LearnigRate": agent.ct_scheduler.get_last_lr()[0]}, step=e)
            vessl.log(payload={"Critic_Train/LearnigRate": agent.critic_scheduler.get_last_lr()[0]}, step=e)
        else:
            writer.add_scalar("Training/FJSP/LearningRate", agent.fjsp_scheduler.get_last_lr()[0], e)
            writer.add_scalar("Training/CT/LearningRate", agent.ct_scheduler.get_last_lr()[0], e)
            writer.add_scalar("Training/Critic/LearningRate", agent.critic_scheduler.get_last_lr()[0], e)

        step = 0
        episode_reward = 0.0
        fjsp_episode_average_loss = 0.0
        ct_episode_average_loss = 0.0
        critic_episode_average_loss = 0.0

        fjsp_state, global_state = env.reset()

        while True:
            mode = "fjsp" if env.scheduling_mode == "machine" else "ct"

            if mode == "fjsp":
                fjsp_action, fjsp_log_prob, value = agent.get_action(local_state=fjsp_state,
                ...
                next_fjsp_state, next_global_state, ct_reward, done = env.step(ct_action)
                episode_reward += ct_reward

            if mode == "fjsp":
                ct_state = next_ct_state
            else:
                agent.put_sample(fjsp_state=fjsp_state,
                                 ct_state=ct_state,
                                 global_state=global_state,
            ...
            if done or len(agent.memory.fjsp_actions) == T_horizon:
                ...
                if len(agent.memory.fjsp_actions) > 0:
                    fjsp_loss, ct_loss, critic_loss = agent.train(last_value)
                    fjsp_episode_average_loss += fjsp_loss
                    ct_episode_average_loss += ct_loss
                    critic_episode_average_loss += critic_loss
                else:
                    agent.memory.clear()
        ...
        print("episode: %d | reward: %.4f | fjsp_loss: %.4f | ct_loss: %.4f | critic_loss: %.4f"
              % (e, episode_reward, fjsp_episode_average_loss / step, ct_episode_average_loss / step, critic_episode_average_loss / step))

        with open(log_dir + "fjsp_train_log.csv", 'a') as f:
            f.write('%d, %1.4f, %1.4f, %f\n'
                    % (e, episode_reward, fjsp_episode_average_loss / step, agent.fjsp_scheduler.get_last_lr()[0]))

        with open(log_dir + "ct_train_log.csv", 'a') as f:
            f.write('%d, %1.4f, %1.4f, %f\n'
                    % (e, episode_reward, ct_episode_average_loss / step, agent.ct_scheduler.get_last_lr()[0]))

        with open(log_dir + "critic_train_log.csv", 'a') as f:
            f.write('%d, %1.4f, %1.4f, %f\n'
                    % (e, episode_reward, critic_episode_average_loss / step, agent.critic_scheduler.get_last_lr()[0]))

        if use_vessl:
            vessl.log(payload={"Common/Reward": episode_reward,
                               "FJSP_Train/Loss": fjsp_episode_average_loss / step,
                               "CT_Train/Loss": ct_episode_average_loss / step,
                               "Critic_Train/Loss": critic_episode_average_loss / step}, step=e)
        else:
            writer.add_scalar("Common/Reward", episode_reward, e)
            writer.add_scalar("Training/FJSP/Loss", fjsp_episode_average_loss / step, e)
            writer.add_scalar("Training/CT/Loss", ct_episode_average_loss / step, e)
            writer.add_scalar("Training/Critic/Loss", critic_episode_average_loss / step, e)

        agent.fjsp_scheduler.step()
        agent.ct_scheduler.step()
        agent.critic_scheduler.step()

        if e == 1 or e % eval_every == 0:
            average_makespan = evaluate(agent, val_dir)
