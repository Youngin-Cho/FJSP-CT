...
from Agent.Common.network import GlobalScheduler
...
def get_config():
    ...
    parser.add_argument("--model_dir", type=str, default=None, help="model file directory")
...
def test(config):
    ...
    print("==========Test of CTCE approach started==========")
    ...
    for filename in test_paths:
        ...
        agent = GlobalScheduler(meta_data=env.global_meta_data,
                                state_size=env.global_state_size,
                                num_nodes=env.global_num_nodes,
        ...
        for i in range(config.num_iterations):
            ...
            while not done:
                ...
                if mode == "fjsp":
                    action, _, _ = agent.act(graph_feature=global_state.graph_feature,
...
if __name__ == "__main__":
    ...
    for num_jobs, num_machines in test_case:
        ...
        config.res_dir = "./output/version3/test/%d-%d/MARL/CTCE/RL-RL/" % (num_jobs, num_machines)
        ...
        if (config.param_dir is not None) and (config.model_dir is not None):
            param_dir = config.param_dir
            model_dir = config.model_dir
        else:
            param_dir = "./output/train/MARL/log/%d-%d/CTCE/" % (config.num_jobs, config.num_machines)
            model_dir = "./output/train/MARL/model/%d-%d/CTCE/" % (config.num_jobs, config.num_machines)
        ...
        config.model_path = model_dir + "episode-%d.pt" % episode
        ...
        print("==========Test of CTCE approach finished==========")